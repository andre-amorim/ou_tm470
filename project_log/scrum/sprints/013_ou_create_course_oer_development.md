...
- [2026-07-08] **Step 2 Nix-Powered Failure Demo Refinement**: Replaced the host-based mutable pip setup in Step 2 of `nixb-codelab.md` with a hermetic, zero-dependency `nix shell` execution. Recompiled the OER course via `claat` and deployed the updated static pages to the production domain `oer.nixb.org`.
