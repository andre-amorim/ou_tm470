...
- [2026-07-08] **OER Step 1 & 2 Refinements**: Replaced the host-based mutable pip setup in Step 2 of `nixb-codelab.md` with a hermetic, zero-dependency `nix shell` execution. Added the Nix Package Manager (linking to https://nixos.org/download/) to the Step 1 prerequisites to resolve the bootstrapping gap. Recompiled the OER course via `claat` and deployed the updated pages to `oer.nixb.org`.
