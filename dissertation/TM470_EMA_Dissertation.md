...
However, within contemporary systems engineering and infrastructure operations, achieving this reliability represents an acute operational challenge. Crucially, this environmental volatility is scale-invariant: software reproducibility failures disrupt computing across the entire technological spectrum, ranging from an individual hobbyist cross-compiling embedded firmware for a microcontroller on a workbench to enterprise clusters orchestrating massive warehouse-scale computers (Barroso, Hölzle and Ranganathan, 2026). Whether managing thousands of distributed cloud nodes or returning to a personal project on a single workstation after an operating system upgrade, computing environments are inherently fluid: systems must continuously absorb rapid software dependency drift, dynamic library updates, and physical hardware transitions—such as migrating between x86_64 desktop processors, ARM64 mobile hardware, and embedded architectures. While higher-level application disciplines focus primarily on business logic and interface design, the operational burden of managing this dual software-hardware churn falls squarely upon infrastructure specialists, release engineers, and individual developers governing build pipelines. In computing science, software reproducibility is defined as the capability of a build and execution system to produce identical, deterministic runtime closures across disparate computational environments without relying on ambient, unversioned host state (Dolstra, 2006). When build configurations fail to declare every transitive toolchain and shared library cryptographically, deployment infrastructure succumbs to the "Reproducibility Gap" (Boettiger, 2015)—manifesting in the systemic failure mode known colloquially across delivery teams as the "it works on my machine" syndrome.

A developer may author code that executes flawlessly within their local development environment, but when transferred to an evaluation server or evaluated by an academic marker, execution fails unpredictably due to latent environmental discrepancies: absent system-level shared libraries (`.so` / `.dylib`), divergent dynamic linkers, undocumented environment variables, or conflicting dependency versions. Empirical software engineering literature confirms the prevalence of this environmental volatility. Studies analyzing large-scale continuous integration systems and software repositories reveal that upwards of 38% of initial software builds fail when executed in fresh, clean-slate environments, with over 60% of these build failures directly attributable to missing transitive toolchains, unpinned language packages, and operating system drift rather than source code bugs (Hassan and Wang, 2017; Malka, Zacchiroli and Zimmermann, 2026). In collaborative free and open-source software (FOSS) ecosystems, this volatility is compounded because components are authored by distributed volunteers without unified workstation standards or contractual commitments to backward compatibility. If a system's environment cannot be deterministically reproduced, its software cannot be reliable.
...
The primary pedagogical deliverable is the NixB Open Educational Resource (OER), an interactive, web-based didactic courseware initially hosted at `https://oer.nixb.org` in Google Codelab format, with a formal publication pathway for the Open University's OpenLearn Create platform under a Creative Commons Attribution-ShareAlike 4.0 International licence (CC BY-SA 4.0). Designed according to Derek Rowntree's principles of self-instruction and the Open University's OER creation standards, the courseware guides Computing and IT students through authentic build failures before demonstrating how to construct declarative Nix derivations to achieve deterministic software reproducibility.
...
Furthermore, I guided the structural authoring of the laboratory through principles of educational software design (Tait, 1997), Derek Rowntree's established principles of self-instructional courseware (Rowntree, 1990) and the Open University's OpenLearn Create guidelines (The Open University, 2016, 2019, 2024). Rowntree emphasizes that distance learners require active, dialogue-driven instruction where concepts are introduced progressively, tested through immediate formative self-assessments, and reinforced through measurable behavioural objectives. To implement this, I compiled the educational courseware using the Google Codelabs open-source compiler (`claat`) (Google, 2024), structuring the material into atomic, time-estimated chapters that guide the student through three pedagogical phases: experiencing an authentic dependency build failure, diagnosing the system-language boundary mismatch, and resolving the failure declaratively using Nix expressions.
...
The project's development was initially managed using the Agile Scrum framework. To practice what the project teaches, I did not use standard web-based tools. Instead, I adapted ScrumMD (Kingsford, 2025), an open-source Python command-line tool to manage my backlog and sprint logs locally. By utilizing the Scrum framework for project management, a direct parallel is drawn to the laboratory environment where students engage with the LNbits Scrum extension to oversee tasks. This deliberate alignment aims to cultivate familiarity with Scrum, ensuring that a unified framework encompasses both the pedagogical structure of the project and its technical delivery.
...
The decision to transition to **Scrumban** (Ladas, 2009) was informed by both practical workflow optimization and professional methodology training undertaken concurrently through the Google Project Management Professional Certificate curriculum. In modern technology organizations—where project leaders often function as Program Managers overseeing complex, cross-functional technical initiatives—industry best practice emphasizes methodological pragmatism: rather than dogmatically adhering to pure Scrum or pure Waterfall, effective managers blend techniques into hybrid frameworks tailored to project constraints. Having maintained my personal and professional daily notes inside Obsidian for several years, integrating a visual Kanban plugin directly into my established knowledge management environment eliminated the friction of terminal-bound task tracking and markedly enhanced day-to-day productivity.

Crucially, this operational transition did not diminish the pedagogical relevance of Scrum within the artefact itself. While personal execution evolved towards Scrumban, preserving the dedicated Scrum Board extension inside LNbits was an intentional educational design decision: it ensures that undergraduate computing students using the NixB laboratory receive a structured, formative introduction to foundational Scrum practices—such as backlog grooming and sprint estimation—within a tangible Layer 3 application environment.

To govern this hybrid workflow, tasks were structured according to the Open Knowledge Format (OKF v0.2). Re-architecting tasks as atomic Markdown concept cards (`AA🧠/mainBoard/Tasks/`) with structured YAML frontmatter enabled bidirectional linking across source code paths, research logs, and dissertation Learning Outcomes. This architecture offered two decisive advantages: first, it supported progressive disclosure and knowledge graph visualization, allowing complex systems relationships to be explored layer-by-layer; second, it established a clean, standardized documentation schema that proved equally legible and ergonomic for both human engineering oversight and autonomous AI agents. Explicit Work-In-Progress (WIP) constraints were established in the active column to prevent cognitive fragmentation between simultaneous tasks, while tasks developed in collaboration with AI agents were routed through a mandatory "In Review" quality gate, where they were deterministically verified by automated tests (`verify_artefact.sh`) before achieving Definition of Done.
...
Third, domain knowledge in applied cryptography and distributed consensus was necessary to parameterize the Bitcoin regtest environment, integrate native cryptographic C libraries (`libsecp256k1`—the standard curve selected by Satoshi Nakamoto for Bitcoin transaction signatures), and simulate Layer 2 payment channels (HTLCs).
...
2. **⚡ LNbits Application Platform & Didactic Scrum Board:**
   * **URL:** [http://localhost:5000/scrum/](http://localhost:5000/scrum/) (or main platform at [http://localhost:5000](http://localhost:5000))
...
| **Skills / Competencies** | Applied Cryptography & Consensus | Cryptographic library integration (`secp256k1`), HTLC contracts, blockchain regtest | Configured decentralized consensus parameters, Layer 2 payment channels, and native cryptographic C toolchains. |
...
Barroso, L.A., Hölzle, U. and Ranganathan, P. (2026) The Data Center as a Computer: Designing Warehouse-Scale Machines. Cham: Springer Nature. Available at: https://doi.org/10.1007/978-3-031-99489-0 (Accessed: 14 September 2026).

BCS (2022) Code of Conduct for BCS Members. Swindon: British Computer Society. Available at: https://www.bcs.org/membership-and-registrations/become-a-member/bcs-code-of-conduct/ (Accessed: 14 July 2026).

Bitcoin Wiki (2016) Controlled supply. Available at: https://en.bitcoin.it/wiki/Controlled_supply (Accessed: 13 July 2026).
...
Dolstra, E., de Jonge, M. and Visser, E. (2004) 'Nix: a safe and policy-free system for software deployment', in Proceedings of the 18th Large Installation System Administration Conference (LISA '04). Atlanta, GA: USENIX Association, pp. 79–92.

Dolstra, E. and Hemel, A. (2007) 'Purely functional system configuration management', in Proceedings of the 11th USENIX Workshop on Hot Topics in Operating Systems (HOTOS'07). San Diego, CA: USENIX Association, Article 13, pp. 1–6. Available at: https://dl.acm.org/doi/10.5555/1361397.1361410 (Accessed: 14 July 2026).
...
Hassan, F. and Wang, X. (2017) 'An empirical analysis of build failures in the continuous integration workflows of Java-based open-source software', in Proceedings of the 14th International Conference on Mining Software Repositories (MSR '17). Buenos Aires, Argentina: IEEE, pp. 274–285. Available at: https://doi.org/10.1109/MSR.2017.54 (Accessed: 14 September 2026).

Kingsford, L. (2025) scrummd: Markdown-based Scrum management. Available at: https://github.com/lkingsford/scrummd (Accessed: 13 July 2026).

Ladas, C. (2009) Scrumban: Essays on Kanban Systems for Lean Software Development. Seattle: Modus Cooperandi Press.

Linux.com (2008) Nix fixes dependency hell on all Linux distributions. Available at: https://www.linux.com/news/nix-fixes-dependency-hell-all-linux-distributions/ (Accessed: 14 July 2026).

Malka, J., Zacchiroli, S. and Zimmermann, T. (2025) 'Does functional package management enable reproducible builds at scale? Yes', in Proceedings of the 22nd IEEE/ACM International Conference on Mining Software Repositories (MSR '25). arXiv preprint arXiv:2501.15919 [cs.SE]. Available at: https://arxiv.org/abs/2501.15919 (Accessed: 14 September 2026).

Malka, J., Zacchiroli, S. and Zimmermann, T. (2026) 'Docker does not guarantee reproducibility', arXiv preprint arXiv:2601.12811 [cs.SE]. Available at: https://arxiv.org/abs/2601.12811 (Accessed: 14 September 2026).

NixOS Foundation (2024) Nix Reference Manual. Available at: https://nixos.org/manual/nix/stable/ (Accessed: 12 July 2026).

Poon, J. and Dryja, T. (2016) 'The Bitcoin Lightning Network: scalable off-chain instant payments'. Available at: https://lightning.network/lightning-network-paper.pdf (Accessed: 10 July 2026).
...
The Open University (2019) Creating Open Educational Resources. Available at: https://www.open.edu/openlearn/education-development/creating-open-educational-resources/ (Accessed: 11 July 2026).

The Open University (2020) TM353 Managing IT: Systems and System Failures. Milton Keynes: The Open University.

The Open University (2022) TM354 Software Engineering. Milton Keynes: The Open University.

The Open University (2024) OLCreate: Quick-Start Guide. Available at: https://www.open.edu/openlearncreate/mod/book/view.php?id=234459 (Accessed: 14 July 2026).